...
def add_all(vacancy_list):
    db_vacancies.insert_many(vacancy_list)


# Добавляем только новые вакансии
def add_new(vacancy_list):
    for item in vacancy_list:
        if db_vacancies.find_one({'link': {'$eq': item['link']}}) is None:
            db_vacancies.insert_one(item)


# Ищем вакансии по указанной зарплате
def find_salary(value):
    for item in db_vacancies.find({'$and': [{'$or': [{'salary_min': {'$ne': None}}, {'salary_max': {'$ne': None}}]},
                                            {'$or': [{'salary_min': {'$gte': value}},
        ...
        for vacancy in vacancies_list:
            ...
            if vacancy_salary:
                ...
                if vacancy_salary.startswith('до'):
                    ...
                    vacancy_salary_max = int(vacancy_salary.partition(' ')[2].rpartition(' ')[0].replace(' ', ''))
                    vacancy_salary_val = vacancy_salary.partition(' ')[2].rpartition(' ')[2]
                elif vacancy_salary.startswith('от'):
                    vacancy_salary_max = None
                    vacancy_salary_min = int(vacancy_salary.partition(' ')[2].rpartition(' ')[0].replace(' ', ''))
                    vacancy_salary_val = vacancy_salary.partition(' ')[2].rpartition(' ')[2]
                else:
                    vacancy_salary_min = int(vacancy_salary.partition(' – ')[0].replace(' ', ''))
                    vacancy_salary_max = int(vacancy_salary.partition(' – ')[2].rpartition(' ')[0].replace(' ', ''))
                    vacancy_salary_val = vacancy_salary.partition(' – ')[2].rpartition(' ')[2]
            else:
    ...
    if response.ok:
        ...
        for vacancy in vacancies_list:
            ...
            if vacancy_salary.startswith('до'):
                ...
                vacancy_salary_min = vacancy_salary_max = int(vacancy_salary.rpartition(' ')[0].replace(' ', ''))
